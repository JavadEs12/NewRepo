namespace ShortestPath
{
    public class HeapBuilder
    {
        ...
        public Heap pointer;
        public int count;
        public HeapBuilder(Dictionary<string, Node> nodes)
        {
            count = 0;
        ...
        public void Add(Node node)
        {
            if (root == null)
            {
                root = new Heap(node.ID, node.Cost);
                count++;
            }
            else
            {
                ...
                for (int i = 1; i < bitcount.Length; i++)   //start from second element of binary to exclude the parent
                {
                    if (bitcount[i] == '0')
                    {
                        ...
                        {
                            pointer.Left = new Heap(pointer); //Node(Node P) is a costructor which creates an empty Node with its Parent
                        }
                    ...
                    else
                    {
                        ...
                        {
                            pointer.Right = new Heap(pointer);
                        }
